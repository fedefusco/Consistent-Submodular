...
'''
kmedoid minimization on the uber Dataset
'''
...
ukmedoid = kmedoid("./datasets/uber/uber_dataset.npz", e_0, k)
n = ukmedoid.n

sieve = sieve_class(ukmedoid,epsilon)
swapping = swapping_class(ukmedoid)
encompassing = encompassing_class(ukmedoid)
chasing = chasing_class(ukmedoid,epsilon)
...
for e in tqdm(stream):
  ...
  S = sieve.solution
  sieve_results.append(ukmedoid.value(S))
...
for e in tqdm(stream):      
  ...
  S = swapping.solution
  swap_results.append(ukmedoid.value(S))
...
for e in tqdm(stream):
  ...
  S = chasing.solution
  chasing_results.append(ukmedoid.value(S))  
...
for e in tqdm(stream):
  ...
  encompassing_results.append(ukmedoid.value(S))

# Saving the result in the /results/uber/kmedoid/results folder
np.savez_compressed("./results/uber/kmedoid/results",
